...
class _AddShiftPageState extends State<AddShiftPage> {
  ...
  void _saveShift() {
    ...
    // [SMART NOTIFICATION] 주휴/목표 알림 즉시 체크
    try {
      final notifProvider =
          Provider.of<NotificationSettingsProvider>(context, listen: false);
      // 현재 주의 누적 시간 (저장된 근무 포함)
      final weeklyHours = provider.getWeeklyWorkedHours(_selectedDate);
      notifProvider.checkWeeklyHoliday(weeklyHours);
      // 이번 달 총 급여
      final monthlySalary = provider.getMonthlyTotalSalary(_selectedDate);
      notifProvider.checkSalaryGoal(monthlySalary);
    } catch (e) {
      debugPrint('[SmartNotification] Auto check failed: $e');
    }
