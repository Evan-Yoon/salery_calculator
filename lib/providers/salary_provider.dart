...
class SalaryProvider with ChangeNotifier {
  // [STUDY NOTE]: 프라이빗 변수(_shifts, _hourlyWage)로 실제 데이터를 안전하게 보관합니다.
  ...
  // [STUDY NOTE]: 등록된 모든 근무 기록의 총 급여를 합산하여 반환하는 Getter
  ...
  Future<void> loadData() async {
    ...
    if (shiftsString != null) {
      try {
        // [STUDY NOTE]: JSON 텍스트를 Map 리스트로 바꾸고, 다시 ShiftEntry 객체 리스트로 변환합니다.
        final List<dynamic> decoded = jsonDecode(shiftsString);
        _shifts = decoded.map((e) => ShiftEntry.fromMap(e)).toList();

        // [STUDY NOTE]: 최신 날짜(가장 최근에 근무한 시간)가 위로 오도록 내림차순 정렬합니다.
  ...
  Future<void> saveData() async {
    ...
    // [STUDY NOTE]: 객체 리스트를 JSON 텍스트로 변환하여 저장합니다.
